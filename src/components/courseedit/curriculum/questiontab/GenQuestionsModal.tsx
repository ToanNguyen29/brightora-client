...
const QuestionsModal: React.FC<QuestionsModalProps> = ({
  ...
  const handleGenerate = async () => {
    try {
      ...
      console.log("genQuestiongenQuestion", response);
  ...
  return (
    <Modal open={open} onClose={onClose}>
      ...
      >
        ...
        >
          {loading ? (
            <CircularProgress size={24} sx={{ color: backgroundColor }} />
          ) : (
            t("generate_question")
          )}
        </Button>
        ...
        <Dialog open={Boolean(selectedQuestion)} onClose={handleCloseDetails}>
          <DialogTitle sx={{ bgcolor: backgroundColor, color: textColor }}>
            {selectedQuestion?.question_text}
          </DialogTitle>
          <DialogContent sx={{ bgcolor: backgroundColor, color: textColor }}>
            <List>
              {selectedQuestion?.choices.map((choice, index) => (
                <ListItem key={index}>
                  <TextField
                    ...
                    sx={{
                      bgcolor: mode === "light" ? "#f0f0f0" : "#2c2c2c",
                      borderRadius: 1,
                    }}
                  />
                </ListItem>
